...
const float DUMP_SEC_PER_FRAME = 0.5 / DUMP_FRAME_PER_SEC;
...
const float SEC_PER_FRAME = 0.5 / 60.0;
...
const float SHOULDER_PITCH_MAX   =  180.0;
...
const float ELBOW_MIN            = -25.0;
const float ELBOW_MAX            =  25.0;

const float KNEE_MIN             = -20.0;
const float KNEE_MAX             =  20.0;

const float MAX_LIGHT            =  90.0;
const float MIN_LIGHT            = -270.0;

const float LIGHT_ANGLE          =  90.0;
const float LIGHT_CIRCLE_RADIUS  =  100;
float LIGHT_SOURCE               =  0.0;

const GLfloat metD[] = { 0.2, 0.2, 0.2, 0.15f };
const GLfloat metA[] = { 0.9, 0.9, 0.9, 1.0f };
const GLfloat metS[] = { 0.7, 0.7, 0.7, 1.0f };

const GLfloat matD[] = { 0.7, 0.7, 0.7, 1.0 };
const GLfloat matA[] = { 0.1, 0.1, 0.1, 0.5 };
const GLfloat matS[] = { 0.1, 0.1, 0.1, 0.5 };
...
void drawTrapPrism(float topWidth);
void drawTrapPrismHelp(float topWidth);
void drawCube();
void drawCubeHelp();
void drawTriPrism();
void drawTriPrismHelp();
...
void initGlui()
{
    ...
    glui_spinner = glui_render->add_spinner_to_panel(glui_panel, "Light Angle: ", GLUI_SPINNER_FLOAT, &LIGHT_SOURCE);
    glui_spinner->set_float_limits(MIN_LIGHT, MAX_LIGHT, GLUI_LIMIT_CLAMP);
    glui_spinner->set_speed(SPINNER_SPEED);
...
void initializeLight() {
    if (renderStyle == METALLIC || renderStyle == MATTE){
        ...
        float light_pos[] = { LIGHT_CIRCLE_RADIUS * cosf(LIGHT_SOURCE * PI / 180),
                LIGHT_CIRCLE_RADIUS * sinf(LIGHT_SOURCE * PI / 180),
                150.0, 1.0 };
...
void display(void)
{
    ...
    const float TORSO_WIDTH = 1.8f;
    const float TORSO_LENGTH = 2.2f;
    const float TORSO_DEPTH = 1.5f;

    const float HEAD_WIDTH = 1.4f;
    const float HEAD_LENGTH = 0.8f;
    const float HEAD_DEPTH = 1.2f;

    const float ARM_WIDTH = 0.6f;
    const float ARM_LENGTH = 1.1f;
    const float ARM_DEPTH = 0.15f;

    const float HAND_WIDTH = ARM_WIDTH*0.7;
    const float HAND_LENGTH = HAND_WIDTH;
    const float HAND_DEPTH = ARM_DEPTH*0.8;

    const float LEG_WIDTH = TORSO_WIDTH/8.0;
    const float LEG_LENGTH = TORSO_LENGTH/2.0;
    const float LEG_DEPTH = TORSO_DEPTH/8.0;

    const float FOOT_WIDTH = TORSO_WIDTH/2.0;
    const float FOOT_LENGTH = TORSO_LENGTH/6.0;
    const float FOOT_DEPTH = TORSO_DEPTH/3.0;

    const float BEAK_WIDTH = HEAD_WIDTH/2.0;
    const float BEAK_LENGTH = HEAD_LENGTH/7.0;
    const float BEAK_DEPTH = HEAD_DEPTH/3.0;
    ...
    case METALLIC:
        glPolygonMode(GL_FRONT_AND_BACK, GL_FILL);
        glMaterialfv(GL_FRONT, GL_DIFFUSE, metD);
        glMaterialfv(GL_FRONT, GL_AMBIENT, metA);
        glMaterialfv(GL_FRONT, GL_SPECULAR, metS);
    ...
    // Update depth buffer (Items behind another won't show)
    glEnable(GL_DEPTH_TEST); 

    glPushMatrix();

        // Setup translations for entire body
        ...
        // Setup rotations for entire body
        ...
        glPushMatrix();
            ...
            glScalef(TORSO_WIDTH, TORSO_LENGTH, TORSO_DEPTH);
            drawTrapPrism(0.3);
        glPopMatrix();
        
        // Draw Entire Head Section -------------------------------------------------------------------------------
        glPushMatrix();

            // Translate to head joint
            glTranslatef(0.0, TORSO_LENGTH/2.0, 0.0);

            // Apply rotation for head
            glRotatef(joint_ui_data->getDOF(Keyframe::HEAD), 0.0, 1.0, 0.0);

            glPushMatrix();

                // Translate head so that bottom of head sits on top of torso (not the center)
                glTranslatef(0.0, HEAD_LENGTH/2.0, 0.0);

                // Draw Head
                glPushMatrix();
                    glColor3f(212.0/255.0, 232.0/255.0, 35.0/255.0);
                    glScalef(HEAD_WIDTH, HEAD_LENGTH, HEAD_DEPTH);
                    drawTrapPrism(0.3);
                glPopMatrix();
                ...
                glPushMatrix();
                    ...
                    // Translate forward to are infront of the head
                    glTranslatef(-HEAD_WIDTH/2.0 - BEAK_WIDTH/5.0, 0.0, 0.0);

                    // Translate postion to top beak
                    glTranslatef(0.0, BEAK_LENGTH/2.0, 0.0);
                    
                    // Top Beak
                    glPushMatrix();
                        // Colour the beak
                        glColor3f(232.0/255.0, 186.0/255.0, 35.0/255.0);
                        // Scale to beak size
                        glScalef(BEAK_WIDTH, BEAK_LENGTH, BEAK_DEPTH);
                        // Draw top beak
                        drawTrapPrism(0.4);
                    glPopMatrix();

                    // Translate postion to bottom of top beak to joint location
                    glTranslatef(0.0, -BEAK_LENGTH/2.0, 0.0);

                    // Apply beak control translation
                    glTranslatef(0.0, - joint_ui_data->getDOF(Keyframe::BEAK), 0.0);

                    // Bottom Beak
                    glPushMatrix();
                        // Translate postion to bottom additionally to align top and bottom beak
                        glTranslatef(0.0, -BEAK_LENGTH/2.0, 0.0);
                        ...
                        glPushMatrix();
                            ...
                            // Scale to beak sizes
                            glScalef(BEAK_WIDTH, BEAK_LENGTH, BEAK_DEPTH);
                            // Rotate 180 on x axis to turn it upside down
        ...
        // Draw Entire Left Arm Section -------------------------------------------------------------------------------
        glPushMatrix();

            // Translate from center to left of torso: z -> TORSO_DEPTH/2.0 + ARM_DEPTH/2.0
            // Translate up to arm joint position: y -> TORSO_LENGTH/4.0
            glTranslatef(0.0, TORSO_LENGTH/4.0, TORSO_DEPTH/2.0 - ARM_DEPTH);
            
            // Apply rotations for shoulder yaw
            glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_YAW), 0.0, 1.0, 0.0);
            // Set default pitch angle at negative seven degrees
            glRotatef(-7.0, 1.0, 0.0, 0.0);
            // Apply rotations for shoulder pitch
            glRotatef(- joint_ui_data->getDOF(Keyframe::L_SHOULDER_PITCH), 1.0, 0.0, 0.0);
            // Apply rotations for shoulder roll
            glRotatef(joint_ui_data->getDOF(Keyframe::L_SHOULDER_ROLL), 0.0, 0.0, 1.0);
            
            glPushMatrix();
                // Translate down so top of arm is near joint (not the center)
                glTranslatef(0.0, -ARM_LENGTH/2.0, 0.0);
            
                // Draw Left Arm
                glPushMatrix();
                    glColor3f(212.0/255.0, 232.0/255.0, 35.0/255.0);
                    // Scale to Arm Dimensions
                    glScalef(ARM_WIDTH, ARM_LENGTH, ARM_DEPTH);
                ...
                glPushMatrix();
                    ...
                    // Translate to bottom of arm to elbow joint
                    glTranslatef(0.0, -ARM_LENGTH/2.0, 0.0);

                    // Apply rotation for elbow
                    glRotatef(joint_ui_data->getDOF(Keyframe::L_ELBOW), 0.0, 0.0, 1.0);

                    glPushMatrix();
                        // Translate down so top of hand is near elbow joint (not the center)
                        glTranslatef(0.0, -HAND_LENGTH/3.0, 0.0);

                        // Scale to Hand Dimensions
                        glScalef(HAND_WIDTH, HAND_LENGTH, HAND_DEPTH);
                        drawCube();
                    glPopMatrix();
        ...
        // Draw Entire Right Arm Section-------------------------------------------------------------------------------
        glPushMatrix();

            // Translate from center to right of torso: z -> - TORSO_DEPTH/2.0 - ARM_DEPTH/2.0
            // Translate up to arm joint position: y -> TORSO_LENGTH/4.0
            glTranslatef(0.0, TORSO_LENGTH/4.0, - TORSO_DEPTH/2.0 + ARM_DEPTH);
            
            // Apply rotations for shoulder yaw
            glRotatef(joint_ui_data->getDOF(Keyframe::R_SHOULDER_YAW), 0.0, 1.0, 0.0);
            // Set default pitch angle at 7 degrees
            glRotatef(7.0, 1.0, 0.0, 0.0);
            // Apply rotations for shoulder pitch
        ...
        // Draw Entire Left Leg Section ---------------------------------------------------------------------------------
        glPushMatrix();
            
            // Translate from center to bottom of torso to leg joint: y -> -TORSO_LENGTH/2.0
            // Translate from center of torso bottom towards left side: z -> TORSO_DEPTH/4.0
            glTranslatef(0.0, - TORSO_LENGTH/2.0, TORSO_DEPTH/4.0);
            
            // Apply rotations for hip yaw
            glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_YAW), 0.0, 1.0, 0.0);
            // Apply rotations for hip pitch
            glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_PITCH), 1.0, 0.0, 0.0);
            // Apply rotations for hip roll
            glRotatef(joint_ui_data->getDOF(Keyframe::L_HIP_ROLL), 0.0, 0.0, 1.0);

            // Draw Left Leg
            glPushMatrix();
                
                // Translate down so top of leg is near joint (not the center)
                glTranslatef(0.0, -LEG_LENGTH/2.0, 0.0);
            ...
            glPushMatrix();
                ...
                // Translate down to leg joint: y -> -LEG_LENGTH
                glTranslatef(0.0, -LEG_LENGTH, 0.0);

                // Apply rotations for knee
                ...
                glPushMatrix();
                    ...
                    // Translate down so top of foot is near joint (not the center)
                    // Translate foot forward so that back of foot aligns with leg
                    glTranslatef(- FOOT_WIDTH/2.0 + LEG_WIDTH, -FOOT_LENGTH/2.0, 0.0);

                    glScalef(FOOT_WIDTH, FOOT_LENGTH, FOOT_DEPTH);
                    drawTriPrism();
                glPopMatrix();
        ...
        // Draw Entire Right Leg Section ---------------------------------------------------------------------------------
        glPushMatrix();
            
            // Translate from center to bottom of torso to leg joint: y -> -TORSO_LENGTH/2.0
            // Translate from center of torso bottom towards right side: z -> TORSO_DEPTH/4.0
            glTranslatef(0.0, - TORSO_LENGTH/2.0, - TORSO_DEPTH/4.0);
...
void drawTrapPrismHelp(float topWidth) {
    glBegin(GL_QUADS);
        ...
        glVertex3f( 0.5, -0.5, 0.5);
        glVertex3f( topWidth,  0.5, topWidth);
        glVertex3f(-topWidth,  0.5, topWidth);
...
void drawTriPrismHelp() {
    glBegin(GL_QUADS);
        ...
        glNormal3f(0.0f, 0.0f, 1.0f);
        glVertex3f(  -0.5, -0.5,  0.5);
        glVertex3f( 0.5, -0.5,  0.0); //p5
        glVertex3f( 0.5,  0.5,  0.0); //p6
        glVertex3f(  -0.5,  0.5,  0.5);
        ...
        glVertex3f( -0.5,  0.5,  0.5); //p4
        glVertex3f( 0.5,  0.5,  0.0); //p6
        glVertex3f( -0.5,  0.5, -0.5); //p3 // double point 3 because we need triangle, but arg is glquads
        ...
        glVertex3f( -0.5, -0.5,  0.5); //p1
        glVertex3f( 0.5, -0.5,  0.0); //p5
        glVertex3f( -0.5, -0.5, -0.5); //p2 // double point 2 because we need triangle, but arg is glquads
    glEnd();
}
